...
from business_logic.calculation.core_stat_calculator import calculate_core_stats
...
class CoreStats(Extension):
    ...
    async def stats(
        ...
        print(
            f"calculating stats for {time_range},{ctx.user.display_name},{playlist},{match_type},{cap}..."
        )
        message = await ctx.send("Roggo Stats is thinking...")
        try:
            result = await calculate_core_stats(
            ...
            await message.edit(
                content="",
                embed=embedder.create_basic_embed(result),
